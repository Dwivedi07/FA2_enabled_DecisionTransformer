...
'''
THe subset data with only K timesteps extracted out of the total 100 timsteps
'''
...
def get_train_val_test_data(mdp_constr, timestep_norm):
    ...
    test_loader = DataLoader(
        test_dataset,
        sampler=torch.utils.data.RandomSampler(
            test_dataset, replacement=True, num_samples=int(5000)),
        shuffle=False,
        pin_memory=True,
        batch_size=1,
        num_workers=0,
...
#     """Add CUDA timing hooks to measure attention vs FFN (MLP) time"""
    
#     timing_data = {'attention_time': 0.0, 'ffn_time': 0.0}
#     event_pairs = {}  # maps module name to (start_event, end_event)
#     handles = []
...
#     def make_post_hook(name, key):
#         def post_hook(module, input, output):
#             end = torch.cuda.Event(enable_timing=True)
#             end.record()
#             torch.cuda.synchronize()
#             start, _ = event_pairs[name]
#             event_pairs[name][1] = end
#             elapsed = start.elapsed_time(end)  # in ms
#             timing_data[key] += elapsed
#         return post_hook

#     for name, module in model.named_modules():
#         if 'attn' in name:
#             handles.append(module.register_forward_pre_hook(make_pre_hook(name)))
#             handles.append(module.register_forward_hook(make_post_hook(name, 'attention_time')))
#         elif 'mlp' in name or 'c_fc' in name:
#             handles.append(module.register_forward_pre_hook(make_pre_hook(name)))
#             handles.append(module.register_forward_hook(make_post_hook(name, 'ffn_time')))

#     return timing_data, handles
...
def torch_model_inference_dyn(model, test_loader, data_sample, rtg_perc=1., ctg_perc=1., rtg=None, ctg_clipped=True):
    ...
    # Retrieve decoded states and actions for different inference cases
    # Initialize tensors: (R, T, dim)
    R = 8 # 32 rollouts in parallel
    states_dyn = torch.zeros(R, n_time, n_state, device=device)
    actions_dyn = torch.zeros(R, n_time, n_action, device=device)
    goals_dyn = torch.zeros(R, n_time, n_state, device=device)
    dv_dyn = torch.zeros(R, n_time, n_action, device=device)
    xypsi_dyn = torch.zeros(R, n_time, n_state, device=device)
    rtgs_dyn = torch.zeros(R, n_time, 1, device=device)
    if test_loader.dataset.mdp_constr:
        ctgs_dyn = torch.zeros(R, n_time, 1, device=device)

    # Dynamics-in-the-loop initialization
    # Initial state at t=0 replicated R times
    states_dyn[:, 0, :] = states_i[:, 0, :].expand(R, -1)
    goals_dyn[:, :, :] = goal_i.expand(R, -1, -1)
    if rtg is None:
    ...
    timesteps_dyn = timesteps_i[:, :1].expand(R, 1).clone()
    attention_mask_dyn = attention_mask_i[:, :1].expand(R, 1).clone()
    ...
    with torch.no_grad():
        _ = model(
            ...
            goal=goals_dyn[:, :1, :],
            returns_to_go=rtgs_dyn[:, :1, :],
            constraints_to_go=ctgs_dyn[:, :1, :] if test_loader.dataset.mdp_constr else None,
            timesteps=timesteps_dyn,
            attention_mask=attention_mask_dyn,
            return_dict=False,
        )
    ...
    for t in np.arange(n_time):
        ##### Dynamics inference        
        # Compute action pred for dynamics model
        attention_mask_dyn = attention_mask_i[:, :t+1].expand(R, t+1).clone()
        # timing_data, hooks = add_timing_hooks(model)
        with torch.no_grad():
            output_dyn = model(
                ...
                constraints_to_go=ctgs_dyn[:, :t+1, :] if test_loader.dataset.mdp_constr else None,
                timesteps=timesteps_dyn[:, :t+1],
                attention_mask=attention_mask_dyn,
                return_dict=False,
            )
            if test_loader.dataset.mdp_constr:
                _, action_mean, action_std = output_dyn
            else:
                _, action_mean, action_std, _ = output_dyn
        
        # print(f"[Timing] Attention: {timing_data['attention_time']:.2f} ms, "
        #     f"MLP: {timing_data['ffn_time']:.2f} ms")
        # # Remove hooks when done
        # for h in hooks:
        #     h.remove()

        # print('Shape of action_mean and action_std',action_mean.shape, action_std.shape)
        curr_mean = action_mean[:, t, :]
        curr_std = action_std  # fixed std across all timesteps
        # curr_std = action_std[:, t, :] # dynamic std

        action_dist = model.get_action_distribution(curr_mean, curr_std)
        action_dyn_t = action_dist.rsample()
        # print('Shape of action after sampling',action_dyn_t.shape)
        actions_dyn[:, t, :] = action_dyn_t
        dv_dyn[:, t] = action_dyn_t * (data_stats['actions_std'][t] + 1e-6) + data_stats['actions_mean'][t]

        # Dynamics propagation of state variable
        # print(Ak.shape, xypsi_dyn[:, t].shape, B_imp.shape, dv_dyn[:, t].shape)
        # torch.Size([6, 6]) torch.Size([32, 6]) torch.Size([6, 3]) torch.Size([32, 3]) 
        if t != n_time - 1:
            Ak_batch = Ak.unsqueeze(0)  # shape: (1, 6, 6)
            B_imp_batch = B_imp.unsqueeze(0)  # shape: (1, 6, 3)

            xypsi_t = xypsi_dyn[:, t].unsqueeze(-1)  # (32, 6, 1)
            dv_t = dv_dyn[:, t].unsqueeze(-1)  # (32, 3, 1)

            # matmul will broadcast Ak and B_imp across batch dimension
            next_state = torch.matmul(Ak_batch, xypsi_t) + torch.matmul(B_imp_batch, dv_t)  # (32, 6, 1)

            # Remove last dim: (32, 6)
            next_state = next_state.squeeze(-1)

            xypsi_dyn[:, t+1] = next_state
            states_dyn[:, t+1, :] = (xypsi_dyn[:, t+1] - data_stats['states_mean'][t+1]) / (data_stats['states_std'][t+1] + 1e-6)

            if test_loader.dataset.mdp_constr:
                reward_dyn_t = -torch.norm(dv_dyn[:, t], p=1, dim=1, keepdim=True)
                rtgs_dyn[:, t+1, :] = rtgs_dyn[:, t, :] - reward_dyn_t
                viol_dyn = torch_check_koz_constraint_batch(xypsi_dyn[:, t+1], obs_pos, obs_rad)
                ctgs_dyn[:, t+1, :] = ctgs_dyn[:, t, :] - (viol_dyn if not ctg_clipped else 0)
            else:
    ...
    print('DT_trajectory:', DT_trajectory['xypsi_dyn'].shape, DT_trajectory['dv_dyn'].shape)    
    return DT_trajectory, runtime_DT

def torch_check_koz_constraint_batch(states, obs_positions, obs_radii):
    
    states_xy = states[:, :2]
    dist = torch.norm(states_xy.unsqueeze(1) - obs_positions.unsqueeze(0), p=2, dim=2)
    constr_koz = dist - obs_radii.unsqueeze(0)
    constr_koz_violation = (constr_koz <= 0).sum().item()
...
def torch_model_inference_ol(model, test_loader, data_sample, rtg_perc=1., ctg_perc=1., rtg=None, ctg_clipped=True):
    ...
    with torch.no_grad():
        # Dummy pass to initialize caches and GPU memory    (states, actions, goal, timesteps,**kwg)
        # with torch.autograd.profiler.profile(use_cuda=torch.cuda.is_available()) as prof:
        output_dyn = model(
                    states=states_ol[:,:1,:],
                    actions=actions_ol[:,:1,:],
                    goal=goal_i[:,:1,:],
                    returns_to_go=rtgs_ol[:,:1,:],
                    constraints_to_go=ctgs_ol[:,:1,:],
                    timesteps=timesteps_i[:,:1],
                    attention_mask=attention_mask_i[:,:1],
                    return_dict=False,
                )
    runtime0_DT = time.time()
    # Open-loop initialization
