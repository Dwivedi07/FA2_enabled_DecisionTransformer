...
# Initial parameters
model_name_4_saving = 'checkpoint_ff_flash_with_pretrainedconfig'
...
# # Initialize WandB
# wandb.init(project="Autonomous-Freeflyer-Transformer", name="run_with_flash_pretrainedconfig", config={
#     "learning_rate": 3e-5,
#     "epochs": 1,
...
config = DecisionTransformerConfig(
    ...
    embd_pdrop=0.1,
    attn_pdrop=0.1,
    attn_implementation = "flash_attention_2", # new line added
    torch_dtype = torch.float16  # new line added
    )
model = AutonomousFreeflyerTransformer(config)
model_size = sum(t.numel() for t in model.parameters())
print(f"GPT size: {model_size/1000**2:.1f}M parameters")
model.to(dtype=torch.float16, device = device) # model datatype must be float16
...
for epoch in range(num_train_epochs):
    print('This is the attn_implementation used', model.config._attn_implementation)
    for step, batch in enumerate(train_dataloader, start=0):
        if step >= 12:
            ...
            with accelerator.accumulate(model):
                ...
                # Log to wandb
                # wandb.log({
                #     "train/state_loss": loss_i_state.item(),
                #     "train/action_loss": loss_i_action.item(),
