...
# Different model to be imported here
from decision_transformer.models_flash.decision_transformer import AutonomousStochasticFlashFreeFlyerTransformer
...
def get_DT_model(model_name, train_loader, eval_loader):
    # DT model creation
    model = AutonomousStochasticFlashFreeFlyerTransformer(
        state_dim=6,
        act_dim=3,
        hidden_size=384,
        n_layer=6,
        n_head=6,
        n_positions=1024,  # must be >= 5 * seq_length
        max_ep_len=100,
    ...
    accelerator.load_state(root_folder + '/decision_transformer/saved_files/checkpointsFlash/' + model_name)    # RCGSA MODEL 
    return model.eval()
...
def torch_model_inference_dyn(model, test_loader, data_sample, rtg_perc=1., ctg_perc=1., rtg=None, ctg_clipped=True):
    ...
    for t in np.arange(n_time):
        ...
        # curr_std = action_std  # static std 
        curr_std = action_std[:, t, :] # dynamic std

        # Option 1: Scale std down
        # curr_std = 0.05 * curr_std
        # Option 2: Clamp std
        # curr_std = torch.clamp(curr_std, min=1e-3, max=0.05)
