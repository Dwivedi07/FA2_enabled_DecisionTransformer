...
class AutonomousFreeflyerTransformer(DecisionTransformerPreTrainedModel):
    ...
    """
    # _supports_flash_attn_2 = True  # explictyl setting this to ask for flash attn 2
...
class AutonomousFreeflyerTransformer2(DecisionTransformerPreTrainedModel):
    """
    ...
    def forward(
        ...
        # which works nice in an autoregressive sense since states predict actions
        stacked_inputs = (
            torch.stack((goal_embeddings, state_embeddings, action_embeddings), dim=1)
            .permute(0, 2, 1, 3)
        ...
        # get predictions
        state_preds = self.predict_state(x[:, -1])  # predict next state given (T+R+C)+state and action
        action_preds = self.predict_action(x[:, -2])  # predict next action given (T+R+C)+state
        if not return_dict:
            return (state_preds, action_preds)
...
class AutonomousStochasticFreeflyerTransformer(DecisionTransformerPreTrainedModel):
    """
    ...
    def __init__(self, config):
        ...
        self.action_mean = nn.Linear(config.hidden_size, config.act_dim)
        self.action_log_std_layer = nn.Linear(config.hidden_size, config.act_dim) # Dynamic std
        # self.action_log_std = nn.Parameter(torch.zeros(config.act_dim))  # fixed std
    ...
    def get_action_distribution(self, action_mean, action_std, action_tanh = False):
        if action_tanh:
...
class AutonomousFreeflyerTransformer_pred_time(DecisionTransformerPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.embed_return = torch.nn.Linear(1, config.hidden_size)
        self.embed_constraint = torch.nn.Linear(1, config.hidden_size)
        self.embed_time = torch.nn.Linear(1, config.hidden_size)
        self.embed_state = torch.nn.Linear(config.state_dim, config.hidden_size)
        self.embed_action = torch.nn.Linear(config.act_dim, config.hidden_size)
        ...
        # note: we don't predict states or returns for the paper
        self.predict_state = torch.nn.Linear(config.hidden_size, config.state_dim)
        self.predict_time = torch.nn.Linear(config.hidden_size, 1)
        self.predict_action = nn.Sequential(
            *([nn.Linear(config.hidden_size, config.act_dim)] + ([nn.Tanh()] if config.action_tanh else []))
        )
